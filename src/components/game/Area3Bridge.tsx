...
import { PixelHeart, PixelBridgePiece, PixelCloud, PixelStar, PixelLock, PixelMoon } from './PixelSprites';
...
export default function Area3Bridge({ unlocked, bridgePieces, bridgePiecesNeeded, complete, onAddPiece, narrationEnabled, onPlayNarration, isNarrationPlaying, isNarrationLoading }: Props) {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-4 py-16" style={{ background: 'linear-gradient(180deg, hsl(240 25% 10%), hsl(260 20% 8%))' }}>
      ...
      <div className="mb-4"><PixelMoon size={5} /></div>
      ...
      <div className="relative w-full max-w-xs h-40">
        ...
        <div className="absolute bottom-0 left-16 right-16 flex justify-center gap-4">
          <div className="opacity-30"><PixelCloud size={3} /></div>
          <div className="opacity-20 mt-2"><PixelCloud size={2} /></div>
        </div>
      </div>
      <div className="relative w-full h-32 mt-8">
        {pieces.map(p => (
          <button key={p.id} onClick={() => handlePlace(p.id)} disabled={placed.has(p.id)}
            className={`absolute transition-all duration-500 select-none ${placed.has(p.id) ? 'opacity-0 scale-0' : 'animate-bounce-gentle cursor-pointer active:scale-125'}`}
            style={{ left: `${p.x}%`, top: `${p.y}%`, animationDelay: `${p.id * 0.3}s` }}>
            <PixelHeart size={4} />
          </button>
        ))}
      </div>

...
export default function Area3Bridge({ unlocked, bridgePieces, bridgePiecesNeeded, complete, onAddPiece }: Props) {
  ...
  if (!unlocked) {
    return (
      <section className="relative min-h-[50vh] flex items-center justify-center"
        style={{ background: 'linear-gradient(180deg, hsl(330 20% 12%), hsl(240 25% 10%))' }}>
        <div className="font-pixel text-[8px] text-muted-foreground/40 animate-pulse-glow">
          🔒 Collect petals above to continue...
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-4 py-16"
      style={{ background: 'linear-gradient(180deg, hsl(240 25% 10%), hsl(260 20% 8%))' }}>

      {/* Stars */}
      {Array.from({ length: 25 }).map((_, i) => (
        <div
          key={i}
          className="absolute w-1 h-1 rounded-full bg-foreground/40 animate-sparkle pointer-events-none"
          style={{
      ...
      <NarrativeText
        text="The distance feels far... but never too far for Buba and Bubibu."
        visible={true}
        className="mb-8"
      />
      ...
      <div className="relative w-full max-w-xs h-40">
        ...
        <div className="absolute bottom-12 left-16 right-16 h-6 flex">
          {Array.from({ length: bridgePiecesNeeded }).map((_, i) => (
            <div
              key={i}
              className={`flex-1 h-full border border-pixel-pink/20 transition-all duration-700 ${
                i < bridgePieces
                  ? 'bg-pixel-pink/40 pixel-border-sm'
        ...
        <div className="absolute bottom-0 left-16 right-16 flex justify-center gap-2">
          ...
          <span className="text-sm opacity-20 mt-2">☁️</span>
        </div>
      </div>
      ...
      <div className="relative w-full h-32 mt-8">
        {pieces.map(p => (
          <button
            ...
            className={`absolute text-2xl transition-all duration-500 select-none ${
              placed.has(p.id)
                ? 'opacity-0 scale-0'
                : 'animate-bounce-gentle cursor-pointer active:scale-125'
            }`}
            style={{
              left: `${p.x}%`,
              top: `${p.y}%`,
      ...
      <div className="font-pixel text-[8px] text-pixel-gold mt-4">
        {complete ? '🌉 Bridge repaired! Continue...' : `💝 Tap hearts to repair: ${bridgePieces}/${bridgePiecesNeeded}`}
      </div>

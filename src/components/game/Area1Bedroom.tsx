...
export default function Area1Bedroom({ heartsCollected, heartsNeeded, complete, onCollectHeart }: Props) {
  ...
  return (
    <section className="relative min-h-screen bg-pixel-night flex flex-col items-center justify-center overflow-hidden px-4 py-16">
      ...
      {Array.from({ length: 30 }).map((_, i) => (
        <div
          key={`rain-${i}`}
          className="absolute w-[1px] h-4 bg-pixel-sky/30 animate-rain pointer-events-none"
          style={{
      ...
      <div className="relative w-56 h-20 bg-muted/20 rounded-lg pixel-border-sm mb-6">
        ...
        <div className="font-pixel text-[7px] text-muted-foreground absolute bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap">
          Bubibu's bed
        </div>
      </div>

      <NarrativeText
        text="Another rainy night... Bubibu stares at the ceiling, thinking of Buba."
        visible={true}
      />
      ...
      <div className="relative w-full h-48 mt-6">
        {hearts.map(h => (
          <button
            ...
            disabled={tapped.has(h.id)}
            className={`absolute transition-all duration-300 select-none ${
              tapped.has(h.id)
                ? 'opacity-0 scale-0'
                : 'animate-float-slow active:scale-150 cursor-pointer'
            }`}
            style={{
              left: `${h.x}%`,
              top: `${h.y}%`,
      ...
      {complete && (
        <div className="mt-8 flex flex-col items-center gap-2 animate-slide-up">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={i}
              className="w-3 h-3 rounded-full bg-pixel-pink animate-pulse-glow"

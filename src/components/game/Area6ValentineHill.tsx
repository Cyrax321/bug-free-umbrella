  ...
  if (!unlocked) {
    return (
      <section className="relative min-h-[50vh] flex items-center justify-center" style={{ background: 'linear-gradient(180deg, hsl(280 25% 10%), hsl(15 30% 15%))' }}>
        <div className="font-pixel text-[8px] text-muted-foreground/40 animate-pulse-glow">🔒 Sync the hearts above to continue...</div>
      </section>
    );
  }
    ...
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-4 py-16" style={{ background: 'linear-gradient(180deg, hsl(15 40% 20%), hsl(340 50% 30%), hsl(15 60% 40%))' }}>
      ...
      {(celebrationActive || wishMade) && Array.from({ length: 20 }).map((_, i) => (
        <div key={i} className="absolute animate-celebration pointer-events-none"
          style={{ left: `${20 + Math.random() * 60}%`, top: `${30 + Math.random() * 40}%`, fontSize: `${14 + Math.random() * 16}px`, animationDelay: `${Math.random()}s` }}>
      ...
      <div className="flex gap-4 mb-8 relative z-10">
        {Array.from({ length: lanternsNeeded }).map((_, i) => (
          <button key={i} onClick={() => { if (i <= lanternsLit) onLightLantern(); }} disabled={i < lanternsLit}
            className={`text-2xl transition-all duration-500 ${i < lanternsLit ? 'animate-flicker' : i === lanternsLit ? 'opacity-60 cursor-pointer active:scale-125' : 'opacity-20'}`}>
            {i < lanternsLit ? '🏮' : '🔲'}
          </button>
        ))}
      </div>
      <div className="relative z-10 text-center mb-6">
        <div className="flex items-end justify-center gap-2 mb-2"><span className="text-xl">🌸</span><span className="text-2xl">🎂</span><span className="text-xl">🌸</span></div>
      ...
      {allLanternsLit && !wishMade && (
        <button onClick={handleWish} className="relative z-10 font-pixel text-xs bg-primary text-primary-foreground px-8 py-4 rounded-lg pixel-border glow-pink active:scale-95 transition-transform animate-pulse-glow">Make Our Wish 💫</button>
      )}
      {!allLanternsLit && <div className="font-pixel text-[8px] text-pixel-gold relative z-10">🏮 Light the lanterns: {lanternsLit}/{lanternsNeeded}</div>}
      {wishMade && (
        <div className="relative z-10 text-center animate-slide-up mt-6">
          ...
          <div className="mt-8 font-pixel text-[7px] text-muted-foreground/60">Made with love by Bubibu 💕</div>
        </div>
      )}
    </section>
  );
